...
function StockDetailPage({ params }: { params: { symbol: string } }) {
  const [isFavorite, setIsFavorite] = useState(false)
  const symbol = params.symbol.toUpperCase()
  const details = mockStockDetails[symbol as keyof typeof mockStockDetails]
  ...
  const toggleFavorite = () => {
    const favorites = JSON.parse(localStorage.getItem("favoriteStocks") || "[]")
    const newFavorites = isFavorite ? favorites.filter((s: string) => s !== symbol) : [...favorites, symbol]
    ...
    return (
      <main className="min-h-screen bg-[#F5F5F5] py-8 px-4">
        <div className="max-w-2xl mx-auto">
          <div className="mb-6">
            <Link href="/stocks" className="inline-flex items-center text-primary hover:underline group">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <div className="text-2xl font-bold">${details.price.toFixed(2)}</div>
                  <div
                    className={`flex items-center gap-1 text-sm ${
                      details.change >= 0 ? "text-green-600" : "text-red-600"
                    }`}
                  >
                    {details.change >= 0 ? <TrendingUp className="h-4 w-4" /> : <TrendingDown className="h-4 w-4" />}
                    {details.change > 0 ? "+" : ""}
                    {details.change}%
                  </div>
                </div>
